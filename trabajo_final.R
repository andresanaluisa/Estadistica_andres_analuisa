...
an <- l %>% select("informalidad","salariom") 


formal    <- an %>% filter(informalidad == "formal") %>% pull(salariom)
informal <- an %>% filter(informalidad == "informal") %>% pull(salariom)


# Diferencias entre medias muestraes 

mean(formal) - mean(informal)
...
ggplot(an,aes(x = salariom)) + 
  geom_histogram(aes(y = ..density.., colour = informalidad)) +
  facet_grid(.~ informalidad) +
...
qqnorm(informal, xlab = "", ylab = "",
       main = "informal", col = "firebrick")
qqline(informal)

qqnorm(formal, xlab = "", ylab = "",
       main = "formal", col = "springgreen4")
qqline(formal)


# LOs graficos muestran asimetria hacia la derecha 


library("nortest")
lillie.test(x = formal) # Shapiiro Wilk
lillie.test(x = informal) # Shapiiro Wilk


# Los graficos qqnorm demuestran una asimetria hacia la derecha y el test de normalidad 
# y los test encuentran evidencias significativas de que los datos no proceden de poblaciones con distribución norma
...
ggplot(data = an) +
  geom_boxplot(aes(x = informalidad, y = salariom, colour = informalidad)) +
  theme_bw() + theme(legend.position = "none")


# Dado que no s e cumple con normalidad test Leven o el test no paramétrico de Fligner-Killeen (ambos basados en la mediana).
...
# Homocedasticidad  
library(car)
leveneTest(an$salariom ~ an$informalidad)

# El valor p de la prueba es 2.2e-16 , que es menor que nuestro nivel de significancia de 0.05. 
# Por lo tanto, rechazamos la hipótesis nula y concluimos que la varianza entre los dos grupos no es igual.
...
# Cálculo tamaño de efecto


library(effsize)
cohen.d(formula = salariom ~ informalidad, data = an, paired = FALSE)




#Dado que p-value (2.2e-16) es menor que alpha (0.05) , 
# se dispone de evidencia suficiente para considerar que existe una diferencia 
# entre el salario de las perosnas con empleo formal e informal. 
# El tamaño de efecto medido por d-Cohen es pequeño (0.2296386 (small)).
...
summary(aov(an$salariom~an$informalidad ))
